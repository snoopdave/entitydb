...
import './styles.css';
import './icons/bird-icon'
import './icons/camera-icon'
    ...
    renderEntity(entity: any) {
        return html`
            <div class="entity">
                ...
                ${entity.id && entity.id.startsWith('media') ? html`
                    <img alt="instagram image" width="400" src="http://localhost:9090/data/instagram/${entity.id}">
                ` : html``}
                
                <div>
                    ${entity.type && entity.type === 'INSTAGRAMPOST' ? html`<camera-icon />` : html`` }
                    ${entity.type && entity.type === 'TWEET' ? html`<bird-icon />` : html`` }
                </div>
